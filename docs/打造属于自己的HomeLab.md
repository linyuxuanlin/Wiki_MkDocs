---
id: 打造属于自己的HomeLab
title: 打造属于自己的 HomeLab
---

Homelab 是指在家中搭建的实验室环境，主要用于进行技术实验和学习。它通常包括一些硬件设备，如服务器、网络设备、存储设备等，以及一些虚拟化技术和操作系统，如 VMware、Docker、Linux 等。通过搭建 Homelab，用户可以在自己的环境中，模拟出一个真实的企业级环境，进行各种实验和测试，提高自己的技术水平和应对能力。Homelab 不仅可以用于个人的学习和实验，也可以用于家庭娱乐、数据备份等多种用途。同时，它也是一个非常有趣的爱好，许多技术爱好者都喜欢通过搭建 Homelab 来挑战自己的技术极限。
...
我个人的 Homelab 配置是 **轻量云服务器** + **小主机** + **群晖 NAS**，它们的配置与用途各有千秋：
...
相比于第三方托管，让别人替你保管数据，**自托管（Self-Hosted）** 具有十足的优势，表现在你对个人数据拥有完全的掌控权，可以根据自己的喜好去定制所需，能帮你获取到更多优质的信息源（个人书库、影视库、RSS 服务）。前提是要有一定的时间精力与资金投入，并且拥有一颗不怕折腾的心。

在接下来的一系列文章中，我将介绍一些基础的配置，还有许多有趣的服务。它们大多是基于 Docker 与 Docker-compose 的，因为这样的部署启动方式兼容性极佳，在不同配置的机器上都能做到开箱即用。
