...
El ancho de un escalón se define como 1 **Bit Menos Significativo (LSB, por sus siglas en inglés)**. La resolución de un ADC se expresa normalmente como el número de bits (código de salida digital). Un ADC con una resolución de n bits tiene $2^n$ posibles códigos digitales ($2^n$ niveles de escalón).
