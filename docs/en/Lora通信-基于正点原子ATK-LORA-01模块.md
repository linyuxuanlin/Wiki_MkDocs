...
The ATK-LORA-01 is a small-sized, low-power, low-consumption, high-performance long-range LORA wireless serial module. The module is designed with the efficient ISM band RF SX1278 spread spectrum chip, and the module's operating frequency is from 410MHz to 441MHz, with a 1MHz frequency step channel, totaling 32 channels. Various parameters such as serial port rate, transmission power, air rate, and operating mode can be modified online through AT commands, and firmware upgrade is supported.
...
- Communication Interface: TTL serial, UART serial, 8N1, 8E1, 8O1, with 8 baud rates from 1200-115200 (default 9600, 8N1)
...
| AUX  | ① Output; ② Input | ① Used to indicate the module's operating status, wake up external MCU; ② When powered on, it enters firmware upgrade mode in conjunction with the MD0 pin |
...
The MD0 and AUX pins have two functions, and different states are entered based on their combination. When the module is powered on for the first time, the AUX pin is in input mode. If the MD0 and AUX pins are both connected to 3.3V TTL high level and maintained for 1 second (without changing the pin level), the module will enter firmware upgrade mode and wait for firmware upgrade. Otherwise, it will enter wireless communication mode (the AUX pin will return to output mode to indicate the module's operating status).
...
| Function        | Description              | Method of Entry                   |
| --------------- | ------------------------ | --------------------------------- |
| Configuration   | Module parameter setting | Power on, AUX floating, MD0 high |
...
In wireless communication mode, the AUX pin is set as output to indicate the module's working status.
...
When exiting the configuration mode (MD0=0), the module will reconfigure the parameters. During the configuration process, the AUX pin remains at a high level and then outputs a low level after completion. This indicates that the module has returned to idle state.

## Sleep Time

The sleep time refers to the interval for the receiver to listen. For the transmitter, it is the duration for continuously transmitting the wake-up code. When the module is in "wake-up mode", it automatically adds the wake-up code for configuring the sleep time before the user data. When the module is in "power-saving mode", the configured sleep time is used as the interval for listening.

## Device Modes

### General Mode (Mode 0)

- Transmitter: The module receives user data from the serial port and transmits wireless data packets with a length of 58 bytes. When the user input reaches 58 bytes, the module starts wireless transmission. The user can continue inputting data to be transmitted. If the number of bytes to be transmitted is less than 58, the module waits for 1 byte of time. If there is no further user input, it is considered as the end of data. At this time, the module transmits all the data wirelessly. When the module starts sending the first packet of user data, the AUX pin outputs a high level. After all the data is transmitted through the RF chip and the transmission is started, the AUX pin outputs a low level. This indicates that the last wireless data packet has been transmitted. The user can continue inputting data up to 512 bytes. The data packets sent through the general mode can only be received by modules in the general mode or wake-up mode.
- Receiver: The module keeps the wireless receiving function open and can receive data packets sent by modules in the general mode or wake-up mode. After receiving a data packet, the AUX pin outputs a high level. After a delay of 2-3ms, the module starts sending the wireless data through the TXD pin of the serial port. After all the wireless data is output through the serial port, the AUX pin outputs a low level.
...
- Transmitter: The module is in sleep mode and the serial port is closed. It cannot receive serial port data from an external MCU, so it does not have the function of wireless transmission.
- Receiver: In power-saving mode, the transmitter must work in wake-up mode. The wireless module periodically listens for the wake-up code. Once a valid wake-up code is received, the module will remain in the receiving state, waiting for the entire valid data packet to be received. Then, the AUX pin outputs a high level. After a delay of 2-3ms, the module opens the serial port and sends the received wireless data through TXD. After completion, the AUX pin outputs a low level. The wireless module continues the "sleep-listen" working state. By setting different wake-up times, the module can achieve different response delays and power consumption. Users need to find a balance between communication delay and average power consumption.

### Signal Strength Mode (Mode 3)

The signal strength mode allows users to view the signal strength of both communication parties and evaluate the communication quality.
...
- Two modules with the same address, same channel, and the same wireless rate (not serial baud rate), where one module sends and the other module receives (one sends, one receives).
...
- Modules with the same address, same channel, and the same wireless rate (not serial baud rate), where any module can send and other modules can receive.
- Each module can send/receive.
- Data is completely transparent, what is sent is what is received.
...
After configuring, click "Save Configuration", **disconnect MD0 and then power off**.

Power on the two modules again, open the configuration software, and **check "HEX" (hexadecimal) for both transmit and receive**.

In the send area of A, enter `FF FE 0A 11 12 13 14`, click send, and you will receive `11 12 13 14` in the receive area of B; or in the send area of B, enter `00 00 00 11 12 13`, and you will receive `11 12 13` in the receive area of A.
...
- Broadcast monitoring does not require the addresses to be the same.
- The channel address can be set. When the address is 0xFFFF, it is in broadcast mode; otherwise, it is in directional transmission mode.
...
- [DFRobot Product Information: LORA Module ATK-LORA-01 Downloads and Technical Discussion](http://www.openedv.com/thread-309019-1-1.html)
