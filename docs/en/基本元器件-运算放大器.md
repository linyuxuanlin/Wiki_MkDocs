...
The Common Mode Rejection Ratio (CMRR) refers to the ratio of the common mode voltage range (CMVR) to the variation of the input offset voltage (ΔV_{O_{OS}}) within this range, expressed in dB.
...
Where f is the maximum frequency (usually the bandwidth) and V_{pk} is the maximum peak-to-peak value of the amplified output signal.
...
- Precision operational amplifiers (V_{os} < 1mV): Low offset voltage, or low temperature drift, low noise, low power consumption, wide bandwidth.
